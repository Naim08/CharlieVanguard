...
export default function ChatDetails({
  ...
  useLayoutEffect(() => {
    if (!initialToggle && isOpen && logs && logs.details) {
      const logsIndex = Object.keys(logs.details).length - 1;
      const stepTitle = Object.keys(logs.details)[logsIndex];
      const stepDetails = Object.values(logs.details)[logsIndex];
      if (contentRefs.current[logsIndex] && stepDetails.length > 0) {
        toggleStep(stepTitle, logsIndex);
